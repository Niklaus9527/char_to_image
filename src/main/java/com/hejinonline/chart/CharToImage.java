...
//        this.uniName = "u_" + this.uniName;
    ...
    public void DrawImages() throws Exception {
        while (nowNum <= totalNum) {
            String imgName = folderPath + "/" + nowNum + "_" + uniName + ".jpg";
...
//        Mat mat = OpencvHandle.affineImage(src);//仿射变换
    ...
    private void drawCharImage(String str, String imgName) {
        ...
        //暂时没有打乱为步骤随机,有的步骤打乱之后会报错,eg:先滤波(模糊化)后截取黑色部分字符会报错
        list.add(0);
        list.add(1);
        list.add(4);
        list.add(6);
        ...
        try {
            ...
            Mat dst_mat = null;
            for (Integer i : list) {
                dst_mat = randomOpencv(handle_mat, i.intValue());
                handle_mat = dst_mat.clone();
            }
            Highgui.imwrite(imgName, dst_mat);
        } catch (Exception e) {
